...
Mom<-for( j in 1:length(Individual_ID)){
    count=count+1
  Kurtosis<-append(Kurtosis,kurtosis(PredictDay(Num=Individual_ID[j])/sum(PredictDay(Num=Individual_ID[j]))))
  Skewness<-append(Skewness,skewness(PredictDay(Num=Individual_ID[j])/sum(PredictDay(Num=Individual_ID[j]))))
  Variance<-append(Variance,var(PredictDay(Num=Individual_ID[j])/sum(PredictDay(Num=Individual_ID[j]))))
} ##Probabilities of each flower having their value for each moment.
...
##Convert to vector 
Kurt<-as.vector(Kurtosis)
Skew<-as.vector(Skewness)
Var<-as.vector(Variance)
Start.date<-as.vector(Start.date)
Duration<-as.vector(Duration)


#Trait matrix -- Kurtosis
trait.matrix.raw.k<-as.matrix(Kurt) 
...
trait.matrix.raw.s<-as.matrix(Skew)
...
trait.matrix.raw.v<-as.matrix(Var) 
trait.matrix.v<- scale(trait.matrix.raw.v)
#Rho -- Variance
rho.matrix.v <- t(trait.matrix.v) %*% mating.matrix %*% trait.matrix.v
rho.matrix.v ##Decided not to keep calculating the rho of variance, bc standardizing these values involves taking the variance of the variance. Too confudled. 

#Trait matrix -- Start date
trait.matrix.raw.st<-as.matrix(Start.date) 
...
trait.matrix.raw.d<-as.matrix(Duration)
...
Mom.ONEC<-for( j in 1:length(Individual_ID2)){
    count2=count2+1
  Kurtosis.ONEC<-append(Kurtosis.ONEC,kurtosis(PredictDayONEC(Num=Individual_ID2[j])/sum(PredictDayONEC(Num=Individual_ID2[j]))))
  Skewness.ONEC<-append(Skewness.ONEC,skewness(PredictDayONEC(Num=Individual_ID2[j])/sum(PredictDayONEC(Num=Individual_ID2[j]))))
  Variance.ONEC<-append(Variance.ONEC,var(PredictDayONEC(Num=Individual_ID2[j])/sum(PredictDayONEC(Num=Individual_ID2[j]))))
}
...
##Convert to vector 
Kurt.ONEC<-as.vector(Kurtosis.ONEC)
Skew.ONEC<-as.vector(Skewness.ONEC)
Var.ONEC<-as.vector(Variance.ONEC)
Start.date.ONEC<-as.vector(Start.date.ONEC)
Duration.ONEC<-as.vector(Duration.ONEC)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONEC<-as.matrix(Kurt.ONEC) 
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONEC<-as.matrix(Skew.ONEC)
...
#Trait matrix -- Variance
trait.matrix.raw.v.ONEC<-as.matrix(Var.ONEC) 
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONEC<-as.matrix(Start.date.ONEC) 
...
#Trait matrix -- Duration
trait.matrix.raw.d.ONEC<-as.matrix(Duration.ONEC)
...
Mom.MDDB<-for( j in 1:length(Individual_ID3)){
    count3=count3+1
  Kurtosis.MDDB<-append(Kurtosis.MDDB,kurtosis(PredictDayMDDB(Num=Individual_ID3[j])/sum(PredictDayMDDB(Num=Individual_ID3[j]))))
  Skewness.MDDB<-append(Skewness.MDDB,skewness(PredictDayMDDB(Num=Individual_ID3[j])/sum(PredictDayMDDB(Num=Individual_ID3[j]))))
  Variance.MDDB<-append(Variance.MDDB,var(PredictDayMDDB(Num=Individual_ID3[j])/sum(PredictDayMDDB(Num=Individual_ID3[j]))))
}
...
##Get z vector 
Kurt.MDDB<-as.vector(Kurtosis.MDDB)
Skew.MDDB<-as.vector(Skewness.MDDB)
Var.MDDB<-as.vector(Variance.MDDB)
Start.date.MDDB<-as.vector(Start.date.MDDB)
Duration.MDDB<-as.vector(Duration.MDDB)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.MDDB<-as.matrix(Kurt.MDDB) 
...
#Trait matrix -- Skewness
trait.matrix.raw.s.MDDB<-as.matrix(Skew.MDDB)
...
## STOPPED CALCULATING VARIANCE AFTER THIS BC DECIDED NOT TO USE IN RESULTS.
trait.matrix.raw.v.MDDB<-as.matrix(Var.MDDB) 
...
#Trait matrix -- Start date
trait.matrix.raw.st.MDDB<-as.matrix(Start.date.MDDB) 
...
#Trait matrix -- Duration
trait.matrix.raw.d.MDDB<-as.matrix(Duration.MDDB)
...
Mom.ONRO<-for( j in 1:length(Individual_ID4)){
    count4=count4+1
  Kurtosis.ONRO<-append(Kurtosis.ONRO,kurtosis(PredictDayONRO(Num=Individual_ID4[j])/sum(PredictDayONRO(Num=Individual_ID4[j]))))
  Skewness.ONRO<-append(Skewness.ONRO,skewness(PredictDayONRO(Num=Individual_ID4[j])/sum(PredictDayONRO(Num=Individual_ID4[j]))))
  Variance.ONRO<-append(Variance.ONRO,var(PredictDayONRO(Num=Individual_ID4[j])/sum(PredictDayONRO(Num=Individual_ID4[j]))))
}
...
##Convert to vector 
Kurt.ONRO<-as.vector(Kurtosis.ONRO)
Skew.ONRO<-as.vector(Skewness.ONRO)
Var.ONRO<-as.vector(Variance.ONRO)
Start.date.ONRO<-as.vector(Start.date.ONRO)
Duration.nONRO<-as.vector(Duration.ONRO)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONRO<-as.matrix(Kurt.ONRO) 
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONRO<-as.matrix(Skew.ONRO)
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONRO<-as.matrix(Start.date.ONRO) 
...
#Trait matrix -- Duration
trait.matrix.raw.d.ONRO<-as.matrix(Duration.ONRO)
