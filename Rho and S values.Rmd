...
dfset0<-dplyr :: select(dfset0,-(Individual.y:Added0)) #Remove unnecessary columns.
...
library(gridExtra)
Gam<-ggplot(dfset0[dfset0$Individual.x=="3",])+
...
Raw<-ggplot(MasterData[MasterData$Individual=="3",]) +
...
Mom.ONEC<-for( j in 1:length(Individual_ID2)){
  ...
  PC1.ONEC<-append(PC1.ONEC,PC1$PC1[PC1$Num==Individual_ID2[j]])
}
...
Mom.MDDB<-for( j in 1:length(Individual_ID3)){
  ...
  PC1.MDDB<-append(PC1.MDDB,PC1$PC1[PC1$Num==Individual_ID3[j]])
}
...
Mom.ONRO<-for( j in 1:length(Individual_ID4)){
  ...
  PC1.ONRO<-append(PC1.ONRO,PC1$PC1[PC1$Num==Individual_ID4[j]])
}
