...
ggplot(Mating.Matrix.Data_Long[Mating.Matrix.Data_Long$Individual %in% c(1:10),])+
...
Mating.Matrix.Data_Wide<-select(Mating.Matrix.Data_Wide,-(Individual))
```
```{r}
...
father.flr.matrix<- apply(flr.matrix, 1, function(d) d/father.colsum) #don't understand this line
...
#Trait matrix
trait.matrix.raw<-as.matrix(Mating.Matrix.Data_Wide[,4:10]) #Why select these rows? Should we be looking for the trait matrix for each of our traits; e.g. variance, skew, kurtosis?
trait.matrix<- scale(trait.matrix.raw)
```
...
###Find start date starting with 1 
Start.date<-as.numeric(names(Mating.Matrix.Data_Wide[-1])[apply(Mating.Matrix.Data_Wide[-1] != 0, 1, which.max)])

###Find end date
End.date<-as.numeric(names(Mating.Matrix.Data_Wide)[-1][max.col(Mating.Matrix.Data_Wide[-1] != 0, 'last')])
...
##Find peak flowering
Peak<-as.numeric(max.col(Mating.Matrix.Data_Wide,ties.method=c("first")))

##Find z -- vector containing the normalized phenotypic value 
Kurt.norm<-as.vector(Kurtosis-mean(Kurtosis))/sd(Kurtosis)
Skew.norm<-as.vector(Skewness-mean(Skewness))/sd(Skewness)
Var.norm<-as.vector(Variance-mean(Variance))/sd(Variance)
Start.date.norm<-as.vector(Start.date-mean(Start.date))/sd(Start.date)
Duration.norm<-as.vector(Duration-mean(Duration))/sd(Duration)
Peak.norm<-as.vector(Peak-mean(Peak))/sd(Peak)

#Trait matrix -- Kurtosis
trait.matrix.raw.k<-as.matrix(Kurt.norm) 
trait.matrix.k<- scale(trait.matrix.raw.k)
#Rho  -- Kurtosis
...
trait.matrix.raw.s<-as.matrix(Skew.norm)
trait.matrix.s<- scale(trait.matrix.raw.s)
#Rho -- Skewness
...
trait.matrix.raw.v<-as.matrix(Var.norm) 
trait.matrix.v<- scale(trait.matrix.raw.v)
#Rho -- Variance
rho.matrix.v <- t(trait.matrix.v) %*% mating.matrix %*% trait.matrix.v
rho.matrix.v ##This value is greater than 1 -- there's something wrong. I found a source online that says it could be bc this character is less heritable than its genotypic correlation, so might be better to remove this particular character from the analysis.
#Perhaps, just create a visual of variance and kurtosis correlating with latitude. 

#Trait matrix -- Start date
trait.matrix.raw.st<-as.matrix(Start.date.norm) 
trait.matrix.st<- scale(trait.matrix.raw.st)
#Rho -- Start date
rho.matrix.st <- t(trait.matrix.st) %*% mating.matrix %*% trait.matrix.st
rho.matrix.st

#Trait matrix -- Duration
trait.matrix.raw.d<-as.matrix(Duration.norm)
trait.matrix.d<- scale(trait.matrix.raw.d)
#Rho -- Duration
rho.matrix.d <- t(trait.matrix.d) %*% mating.matrix %*% trait.matrix.d
rho.matrix.d

#Trait matrix -- Peak flowering
trait.matrix.raw.peak<-as.matrix(Peak.norm)
trait.matrix.peak<-scale(trait.matrix.raw.peak)
#Rho -- Peak flowering
rho.matrix.peak <- t(trait.matrix.peak) %*% mating.matrix %*% trait.matrix.peak
rho.matrix.peak
```
...
###Find start date starting with 1 
Start.date.ONEC<-as.numeric(names(Mating.Matrix.Data.ONEC_Wide[-1])[apply(Mating.Matrix.Data.ONEC_Wide[-1] != 0, 1, which.max)])

###Find end date
End.date.ONEC<-as.numeric(names(Mating.Matrix.Data.ONEC_Wide)[-1][max.col(Mating.Matrix.Data.ONEC_Wide[-1] != 0, 'last')])
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONEC<-as.vector(Kurtosis.ONEC-mean(Kurtosis.ONEC))/sd(Kurtosis.ONEC)
Skew.norm.ONEC<-as.vector(Skewness.ONEC-mean(Skewness.ONEC))/sd(Skewness.ONEC)
Var.norm.ONEC<-as.vector(Variance.ONEC-mean(Variance.ONEC))/sd(Variance.ONEC)
Start.date.norm.ONEC<-as.vector(Start.date.ONEC-mean(Start.date.ONEC))/sd(Start.date.ONEC)
Duration.norm.ONEC<-as.vector(Duration.ONEC-mean(Duration.ONEC))/sd(Duration.ONEC)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONEC<-as.matrix(Kurt.norm.ONEC) 
trait.matrix.k.ONEC<- scale(trait.matrix.raw.k.ONEC)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONEC<-as.matrix(Skew.norm.ONEC)
trait.matrix.s.ONEC<- scale(trait.matrix.raw.s.ONEC)
#Rho -- Skewness
...
#Trait matrix -- Variance
trait.matrix.raw.v.ONEC<-as.matrix(Var.norm.ONEC) 
trait.matrix.v.ONEC<- scale(trait.matrix.raw.v.ONEC)
#Rho -- Variance
rho.matrix.v.ONEC <- t(trait.matrix.v.ONEC) %*% mating.matrix.ONEC %*% trait.matrix.v.ONEC
rho.matrix.v.ONEC ##This value is greater than 1 -- there's something wrong. I found a source online that says it could be bc this character is less heritable than its genotypic correlation, so might be better to remove this particular character from the analysis.
#Perhaps, just create a visual of variance and kurtosis correlating with latitude. 

#Trait matrix -- Start date
trait.matrix.raw.st.ONEC<-as.matrix(Start.date.norm.ONEC) 
trait.matrix.st.ONEC<- scale(trait.matrix.raw.st.ONEC)
#Rho -- Start date
rho.matrix.st.ONEC <- t(trait.matrix.st.ONEC) %*% mating.matrix.ONEC %*% trait.matrix.st.ONEC
rho.matrix.st.ONEC

#Trait matrix -- Duration
trait.matrix.raw.d.ONEC<-as.matrix(Duration.norm.ONEC)
trait.matrix.d.ONEC<- scale(trait.matrix.raw.d.ONEC)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.MDDB<-as.numeric(names(Mating.Matrix.Data.MDDB_Wide[-1])[apply(Mating.Matrix.Data.MDDB_Wide[-1] != 0, 1, which.max)])

###Find end date
End.date.MDDB<-as.numeric(names(Mating.Matrix.Data.MDDB_Wide)[-1][max.col(Mating.Matrix.Data.MDDB_Wide[-1] != 0, 'last')])
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.MDDB<-as.vector(Kurtosis.MDDB-mean(Kurtosis.MDDB))/sd(Kurtosis.MDDB)
Skew.norm.MDDB<-as.vector(Skewness.MDDB-mean(Skewness.MDDB))/sd(Skewness.MDDB)
Var.norm.MDDB<-as.vector(Variance.MDDB-mean(Variance.MDDB))/sd(Variance.MDDB)
Start.date.norm.MDDB<-as.vector(Start.date.MDDB-mean(Start.date.MDDB))/sd(Start.date.MDDB)
Duration.norm.MDDB<-as.vector(Duration.MDDB-mean(Duration.MDDB))/sd(Duration.MDDB)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.MDDB<-as.matrix(Kurt.norm.MDDB) 
trait.matrix.k.MDDB<- scale(trait.matrix.raw.k.MDDB)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.MDDB<-as.matrix(Skew.norm.MDDB)
trait.matrix.s.MDDB<- scale(trait.matrix.raw.s.MDDB)
#Rho -- Skewness
rho.matrix.s.MDDB <- t(trait.matrix.s.MDDB) %*% mating.matrix.MDDB %*% trait.matrix.s.MDDB
rho.matrix.s.MDDB

## STOPPED CALCULATING VARIANCE AFTER THIS BC DECIDED NOT TO USE IN RESULTS. VALUE IS AGAIN GREATER THAN 1.
trait.matrix.raw.v.MDDB<-as.matrix(Var.norm.MDDB) 
trait.matrix.v.MDDB<- scale(trait.matrix.raw.v.MDDB)
#Rho -- Variance
rho.matrix.v.MDDB <- t(trait.matrix.v.MDDB) %*% mating.matrix.MDDB %*% trait.matrix.v.MDDB
rho.matrix.v.MDDB

#Trait matrix -- Start date
trait.matrix.raw.st.MDDB<-as.matrix(Start.date.norm.MDDB) 
trait.matrix.st.MDDB<- scale(trait.matrix.raw.st.MDDB)
#Rho -- Start date
rho.matrix.st.MDDB <- t(trait.matrix.st.MDDB) %*% mating.matrix.MDDB %*% trait.matrix.st.MDDB
rho.matrix.st.MDDB

#Trait matrix -- Duration
trait.matrix.raw.d.MDDB<-as.matrix(Duration.norm.MDDB)
trait.matrix.d.MDDB<- scale(trait.matrix.raw.d.MDDB)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONRO<-as.numeric(names(Mating.Matrix.Data.ONRO_Wide[-1])[apply(Mating.Matrix.Data.ONRO_Wide[-1] != 0, 1, which.max)])
Start.date.ONRO

###Find end date
End.date.ONRO<-as.numeric(names(Mating.Matrix.Data.ONRO_Wide)[-1][max.col(Mating.Matrix.Data.ONRO_Wide[-1] != 0, 'last')])
End.date.ONRO
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONRO<-as.vector(Kurtosis.ONRO-mean(Kurtosis.ONRO))/sd(Kurtosis.ONRO)
Skew.norm.ONRO<-as.vector(Skewness.ONRO-mean(Skewness.ONRO))/sd(Skewness.ONRO)
Var.norm.ONRO<-as.vector(Variance.ONRO-mean(Variance.ONRO))/sd(Variance.ONRO)
Start.date.norm.ONRO<-as.vector(Start.date.ONRO-mean(Start.date.ONRO))/sd(Start.date.ONRO)
Duration.norm.ONRO<-as.vector(Duration.ONRO-mean(Duration.ONRO))/sd(Duration.ONRO)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONRO<-as.matrix(Kurt.norm.ONRO) 
trait.matrix.k.ONRO<- scale(trait.matrix.raw.k.ONRO)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONRO<-as.matrix(Skew.norm.ONRO)
trait.matrix.s.ONRO<- scale(trait.matrix.raw.s.ONRO)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONRO<-as.matrix(Start.date.norm.ONRO) 
trait.matrix.st.ONRO<- scale(trait.matrix.raw.st.ONRO)
#Rho -- Start date
rho.matrix.st.ONRO <- t(trait.matrix.st.ONRO) %*% mating.matrix.ONRO %*% trait.matrix.st.ONRO
rho.matrix.st.ONRO

#Trait matrix -- Duration
trait.matrix.raw.d.ONRO<-as.matrix(Duration.norm.ONRO)
trait.matrix.d.ONRO<- scale(trait.matrix.raw.d.ONRO)
#Rho -- Duration
...
#Probability function -- should be the number of flowers on each day divided by the total 

##Find average variance.
mean(Variance.NYSC)

###Find start date starting with 1 
Start.date.NYSC<-as.numeric(names(Mating.Matrix.Data.NYSC_Wide[-1])[apply(Mating.Matrix.Data.NYSC_Wide[-1] != 0, 1, which.max)]) ##Take out -1
Start.date.NYSC

###Find end date
End.date.NYSC<-as.numeric(names(Mating.Matrix.Data.NYSC_Wide)[-1][max.col(Mating.Matrix.Data.NYSC_Wide[-1] != 0, 'last')])
End.date.NYSC
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.NYSC<-as.vector(Kurtosis.NYSC-mean(Kurtosis.NYSC))/sd(Kurtosis.NYSC)
Skew.norm.NYSC<-as.vector(Skewness.NYSC-mean(Skewness.NYSC))/sd(Skewness.NYSC)
Var.norm.NYSC<-as.vector(Variance.NYSC-mean(Variance.NYSC))/sd(Variance.NYSC)
Start.date.norm.NYSC<-as.vector(Start.date.NYSC-mean(Start.date.NYSC))/sd(Start.date.NYSC)
Duration.norm.NYSC<-as.vector(Duration.NYSC-mean(Duration.NYSC))/sd(Duration.NYSC) #Already scaled traits.

#Trait matrix -- Kurtosis
trait.matrix.raw.k.NYSC<-as.matrix(Kurt.norm.NYSC) 
trait.matrix.k.NYSC<- scale(trait.matrix.raw.k.NYSC)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.NYSC<-as.matrix(Skew.norm.NYSC)
trait.matrix.s.NYSC<- scale(trait.matrix.raw.s.NYSC)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.NYSC<-as.matrix(Start.date.norm.NYSC) 
trait.matrix.st.NYSC<- scale(trait.matrix.raw.st.NYSC)
#Rho -- Start date
rho.matrix.st.NYSC <- t(trait.matrix.st.NYSC) %*% mating.matrix.NYSC %*% trait.matrix.st.NYSC
rho.matrix.st.NYSC

#Trait matrix -- Duration
trait.matrix.raw.d.NYSC<-as.matrix(Duration.norm.NYSC)
trait.matrix.d.NYSC<- scale(trait.matrix.raw.d.NYSC)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.NYLV<-as.numeric(names(Mating.Matrix.Data.NYLV_Wide[-1])[apply(Mating.Matrix.Data.NYLV_Wide[-1] != 0, 1, which.max)])
Start.date.NYLV

###Find end date
End.date.NYLV<-as.numeric(names(Mating.Matrix.Data.NYLV_Wide)[-1][max.col(Mating.Matrix.Data.NYLV_Wide[-1] != 0, 'last')])
End.date.NYLV
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.NYLV<-as.vector(Kurtosis.NYLV-mean(Kurtosis.NYLV))/sd(Kurtosis.NYLV)
Skew.norm.NYLV<-as.vector(Skewness.NYLV-mean(Skewness.NYLV))/sd(Skewness.NYLV)
Var.norm.NYLV<-as.vector(Variance.NYLV-mean(Variance.NYLV))/sd(Variance.NYLV)
Start.date.norm.NYLV<-as.vector(Start.date.NYLV-mean(Start.date.NYLV))/sd(Start.date.NYLV)
Duration.norm.NYLV<-as.vector(Duration.NYLV-mean(Duration.NYLV))/sd(Duration.NYLV)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.NYLV<-as.matrix(Kurt.norm.NYLV) 
trait.matrix.k.NYLV<- scale(trait.matrix.raw.k.NYLV)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.NYLV<-as.matrix(Skew.norm.NYLV)
trait.matrix.s.NYLV<- scale(trait.matrix.raw.s.NYLV)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.NYLV<-as.matrix(Start.date.norm.NYLV) 
trait.matrix.st.NYLV<- scale(trait.matrix.raw.st.NYLV)
#Rho -- Start date
rho.matrix.st.NYLV <- t(trait.matrix.st.NYLV) %*% mating.matrix.NYLV %*% trait.matrix.st.NYLV
rho.matrix.st.NYLV

#Trait matrix -- Duration
trait.matrix.raw.d.NYLV<-as.matrix(Duration.norm.NYLV)
trait.matrix.d.NYLV<- scale(trait.matrix.raw.d.NYLV)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONDM<-as.numeric(names(Mating.Matrix.Data.ONDM_Wide[-1])[apply(Mating.Matrix.Data.ONDM_Wide[-1] != 0, 1, which.max)])
Start.date.ONDM

###Find end date
End.date.ONDM<-as.numeric(names(Mating.Matrix.Data.ONDM_Wide)[-1][max.col(Mating.Matrix.Data.ONDM_Wide[-1] != 0, 'last')])
End.date.ONDM
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONDM<-as.vector(Kurtosis.ONDM-mean(Kurtosis.ONDM))/sd(Kurtosis.ONDM)
Skew.norm.ONDM<-as.vector(Skewness.ONDM-mean(Skewness.ONDM))/sd(Skewness.ONDM)
Var.norm.ONDM<-as.vector(Variance.ONDM-mean(Variance.ONDM))/sd(Variance.ONDM)
Start.date.norm.ONDM<-as.vector(Start.date.ONDM-mean(Start.date.ONDM))/sd(Start.date.ONDM)
Duration.norm.ONDM<-as.vector(Duration.ONDM-mean(Duration.ONDM))/sd(Duration.ONDM)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONDM<-as.matrix(Kurt.norm.ONDM) 
trait.matrix.k.ONDM<- scale(trait.matrix.raw.k.ONDM)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONDM<-as.matrix(Skew.norm.ONDM)
trait.matrix.s.ONDM<- scale(trait.matrix.raw.s.ONDM)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONDM<-as.matrix(Start.date.norm.ONDM) 
trait.matrix.st.ONDM<- scale(trait.matrix.raw.st.ONDM)
#Rho -- Start date
rho.matrix.st.ONDM <- t(trait.matrix.st.ONDM) %*% mating.matrix.ONDM %*% trait.matrix.st.ONDM
rho.matrix.st.ONDM

#Trait matrix -- Duration
trait.matrix.raw.d.ONDM<-as.matrix(Duration.norm.ONDM)
trait.matrix.d.ONDM<- scale(trait.matrix.raw.d.ONDM)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONTI<-as.numeric(names(Mating.Matrix.Data.ONTI_Wide[-1])[apply(Mating.Matrix.Data.ONTI_Wide[-1] != 0, 1, which.max)])
Start.date.ONTI

###Find end date
End.date.ONTI<-as.numeric(names(Mating.Matrix.Data.ONTI_Wide)[-1][max.col(Mating.Matrix.Data.ONTI_Wide[-1] != 0, 'last')])
End.date.ONTI
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONTI<-as.vector(Kurtosis.ONTI-mean(Kurtosis.ONTI))/sd(Kurtosis.ONTI)
Skew.norm.ONTI<-as.vector(Skewness.ONTI-mean(Skewness.ONTI))/sd(Skewness.ONTI)
Var.norm.ONTI<-as.vector(Variance.ONTI-mean(Variance.ONTI))/sd(Variance.ONTI)
Start.date.norm.ONTI<-as.vector(Start.date.ONTI-mean(Start.date.ONTI))/sd(Start.date.ONTI)
Duration.norm.ONTI<-as.vector(Duration.ONTI-mean(Duration.ONTI))/sd(Duration.ONTI)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONTI<-as.matrix(Kurt.norm.ONTI) 
trait.matrix.k.ONTI<- scale(trait.matrix.raw.k.ONTI)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONTI<-as.matrix(Skew.norm.ONTI)
trait.matrix.s.ONTI<- scale(trait.matrix.raw.s.ONTI)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONTI<-as.matrix(Start.date.norm.ONTI) 
trait.matrix.st.ONTI<- scale(trait.matrix.raw.st.ONTI)
#Rho -- Start date
rho.matrix.st.ONTI <- t(trait.matrix.st.ONTI) %*% mating.matrix.ONTI %*% trait.matrix.st.ONTI
rho.matrix.st.ONTI

#Trait matrix -- Duration
trait.matrix.raw.d.ONTI<-as.matrix(Duration.norm.ONTI)
trait.matrix.d.ONTI<- scale(trait.matrix.raw.d.ONTI)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONWH<-as.numeric(names(Mating.Matrix.Data.ONWH_Wide[-1])[apply(Mating.Matrix.Data.ONWH_Wide[-1] != 0, 1, which.max)])
Start.date.ONWH

###Find end date
End.date.ONWH<-as.numeric(names(Mating.Matrix.Data.ONWH_Wide)[-1][max.col(Mating.Matrix.Data.ONWH_Wide[-1] != 0, 'last')])
End.date.ONWH
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONWH<-as.vector(Kurtosis.ONWH-mean(Kurtosis.ONWH))/sd(Kurtosis.ONWH)
Skew.norm.ONWH<-as.vector(Skewness.ONWH-mean(Skewness.ONWH))/sd(Skewness.ONWH)
Var.norm.ONWH<-as.vector(Variance.ONWH-mean(Variance.ONWH))/sd(Variance.ONWH)
Start.date.norm.ONWH<-as.vector(Start.date.ONWH-mean(Start.date.ONWH))/sd(Start.date.ONWH)
Duration.norm.ONWH<-as.vector(Duration.ONWH-mean(Duration.ONWH))/sd(Duration.ONWH)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONWH<-as.matrix(Kurt.norm.ONWH) 
trait.matrix.k.ONWH<- scale(trait.matrix.raw.k.ONWH)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONWH<-as.matrix(Skew.norm.ONWH)
trait.matrix.s.ONWH<- scale(trait.matrix.raw.s.ONWH)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONWH<-as.matrix(Start.date.norm.ONWH) 
trait.matrix.st.ONWH<- scale(trait.matrix.raw.st.ONWH)
#Rho -- Start date
rho.matrix.st.ONWH <- t(trait.matrix.st.ONWH) %*% mating.matrix.ONWH %*% trait.matrix.st.ONWH
rho.matrix.st.ONWH

#Trait matrix -- Duration
trait.matrix.raw.d.ONWH<-as.matrix(Duration.norm.ONWH)
trait.matrix.d.ONWH<- scale(trait.matrix.raw.d.ONWH)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.NJHP<-as.numeric(names(Mating.Matrix.Data.NJHP_Wide[-1])[apply(Mating.Matrix.Data.NJHP_Wide[-1] != 0, 1, which.max)])
Start.date.NJHP

###Find end date
End.date.NJHP<-as.numeric(names(Mating.Matrix.Data.NJHP_Wide)[-1][max.col(Mating.Matrix.Data.NJHP_Wide[-1] != 0, 'last')])
End.date.NJHP
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.NJHP<-as.vector(Kurtosis.NJHP-mean(Kurtosis.NJHP))/sd(Kurtosis.NJHP)
Skew.norm.NJHP<-as.vector(Skewness.NJHP-mean(Skewness.NJHP))/sd(Skewness.NJHP)
Var.norm.NJHP<-as.vector(Variance.NJHP-mean(Variance.NJHP))/sd(Variance.NJHP)
Start.date.norm.NJHP<-as.vector(Start.date.NJHP-mean(Start.date.NJHP))/sd(Start.date.NJHP)
Duration.norm.NJHP<-as.vector(Duration.NJHP-mean(Duration.NJHP))/sd(Duration.NJHP)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.NJHP<-as.matrix(Kurt.norm.NJHP) 
trait.matrix.k.NJHP<- scale(trait.matrix.raw.k.NJHP)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.NJHP<-as.matrix(Skew.norm.NJHP)
trait.matrix.s.NJHP<- scale(trait.matrix.raw.s.NJHP)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.NJHP<-as.matrix(Start.date.norm.NJHP) 
trait.matrix.st.NJHP<- scale(trait.matrix.raw.st.NJHP)
#Rho -- Start date
rho.matrix.st.NJHP <- t(trait.matrix.st.NJHP) %*% mating.matrix.NJHP %*% trait.matrix.st.NJHP
rho.matrix.st.NJHP

#Trait matrix -- Duration
trait.matrix.raw.d.NJHP<-as.matrix(Duration.norm.NJHP)
trait.matrix.d.NJHP<- scale(trait.matrix.raw.d.NJHP)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONNO<-as.numeric(names(Mating.Matrix.Data.ONNO_Wide[-1])[apply(Mating.Matrix.Data.ONNO_Wide[-1] != 0, 1, which.max)])
Start.date.ONNO

###Find end date
End.date.ONNO<-as.numeric(names(Mating.Matrix.Data.ONNO_Wide)[-1][max.col(Mating.Matrix.Data.ONNO_Wide[-1] != 0, 'last')])
End.date.ONNO
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONNO<-as.vector(Kurtosis.ONNO-mean(Kurtosis.ONNO))/sd(Kurtosis.ONNO)
Skew.norm.ONNO<-as.vector(Skewness.ONNO-mean(Skewness.ONNO))/sd(Skewness.ONNO)
Var.norm.ONNO<-as.vector(Variance.ONNO-mean(Variance.ONNO))/sd(Variance.ONNO)
Start.date.norm.ONNO<-as.vector(Start.date.ONNO-mean(Start.date.ONNO))/sd(Start.date.ONNO)
Duration.norm.ONNO<-as.vector(Duration.ONNO-mean(Duration.ONNO))/sd(Duration.ONNO)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONNO<-as.matrix(Kurt.norm.ONNO) 
trait.matrix.k.ONNO<- scale(trait.matrix.raw.k.ONNO)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONNO<-as.matrix(Skew.norm.ONNO)
trait.matrix.s.ONNO<- scale(trait.matrix.raw.s.ONNO)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONNO<-as.matrix(Start.date.norm.ONNO) 
trait.matrix.st.ONNO<- scale(trait.matrix.raw.st.ONNO)
#Rho -- Start date
rho.matrix.st.ONNO <- t(trait.matrix.st.ONNO) %*% mating.matrix.ONNO %*% trait.matrix.st.ONNO
rho.matrix.st.ONNO

#Trait matrix -- Duration
trait.matrix.raw.d.ONNO<-as.matrix(Duration.norm.ONNO)
trait.matrix.d.ONNO<- scale(trait.matrix.raw.d.ONNO)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.NYCH<-as.numeric(names(Mating.Matrix.Data.NYCH_Wide[-1])[apply(Mating.Matrix.Data.NYCH_Wide[-1] != 0, 1, which.max)])
Start.date.NYCH

###Find end date
End.date.NYCH<-as.numeric(names(Mating.Matrix.Data.NYCH_Wide)[-1][max.col(Mating.Matrix.Data.NYCH_Wide[-1] != 0, 'last')])
End.date.NYCH
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.NYCH<-as.vector(Kurtosis.NYCH-mean(Kurtosis.NYCH))/sd(Kurtosis.NYCH)
Skew.norm.NYCH<-as.vector(Skewness.NYCH-mean(Skewness.NYCH))/sd(Skewness.NYCH)
Var.norm.NYCH<-as.vector(Variance.NYCH-mean(Variance.NYCH))/sd(Variance.NYCH)
Start.date.norm.NYCH<-as.vector(Start.date.NYCH-mean(Start.date.NYCH))/sd(Start.date.NYCH)
Duration.norm.NYCH<-as.vector(Duration.NYCH-mean(Duration.NYCH))/sd(Duration.NYCH)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.NYCH<-as.matrix(Kurt.norm.NYCH) 
trait.matrix.k.NYCH<- scale(trait.matrix.raw.k.NYCH)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.NYCH<-as.matrix(Skew.norm.NYCH)
trait.matrix.s.NYCH<- scale(trait.matrix.raw.s.NYCH)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.NYCH<-as.matrix(Start.date.norm.NYCH) 
trait.matrix.st.NYCH<- scale(trait.matrix.raw.st.NYCH)
#Rho -- Start date
rho.matrix.st.NYCH <- t(trait.matrix.st.NYCH) %*% mating.matrix.NYCH %*% trait.matrix.st.NYCH
rho.matrix.st.NYCH

#Trait matrix -- Duration
trait.matrix.raw.d.NYCH<-as.matrix(Duration.norm.NYCH)
trait.matrix.d.NYCH<- scale(trait.matrix.raw.d.NYCH)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.ONPG<-as.numeric(names(Mating.Matrix.Data.ONPG_Wide[-1])[apply(Mating.Matrix.Data.ONPG_Wide[-1] != 0, 1, which.max)])
Start.date.ONPG

###Find end date
End.date.ONPG<-as.numeric(names(Mating.Matrix.Data.ONPG_Wide)[-1][max.col(Mating.Matrix.Data.ONPG_Wide[-1] != 0, 'last')])
End.date.ONPG
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.ONPG<-as.vector(Kurtosis.ONPG-mean(Kurtosis.ONPG))/sd(Kurtosis.ONPG)
Skew.norm.ONPG<-as.vector(Skewness.ONPG-mean(Skewness.ONPG))/sd(Skewness.ONPG)
Var.norm.ONPG<-as.vector(Variance.ONPG-mean(Variance.ONPG))/sd(Variance.ONPG)
Start.date.norm.ONPG<-as.vector(Start.date.ONPG-mean(Start.date.ONPG))/sd(Start.date.ONPG)
Duration.norm.ONPG<-as.vector(Duration.ONPG-mean(Duration.ONPG))/sd(Duration.ONPG)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.ONPG<-as.matrix(Kurt.norm.ONPG) 
trait.matrix.k.ONPG<- scale(trait.matrix.raw.k.ONPG)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.ONPG<-as.matrix(Skew.norm.ONPG)
trait.matrix.s.ONPG<- scale(trait.matrix.raw.s.ONPG)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.ONPG<-as.matrix(Start.date.norm.ONPG) 
trait.matrix.st.ONPG<- scale(trait.matrix.raw.st.ONPG)
#Rho -- Start date
rho.matrix.st.ONPG <- t(trait.matrix.st.ONPG) %*% mating.matrix.ONPG %*% trait.matrix.st.ONPG
rho.matrix.st.ONPG

#Trait matrix -- Duration
trait.matrix.raw.d.ONPG<-as.matrix(Duration.norm.ONPG)
trait.matrix.d.ONPG<- scale(trait.matrix.raw.d.ONPG)
#Rho -- Duration
...
###Find start date starting with 1 
Start.date.NJPT<-as.numeric(names(Mating.Matrix.Data.NJPT_Wide[-1])[apply(Mating.Matrix.Data.NJPT_Wide[-1] != 0, 1, which.max)])
Start.date.NJPT

###Find end date
End.date.NJPT<-as.numeric(names(Mating.Matrix.Data.NJPT_Wide)[-1][max.col(Mating.Matrix.Data.NJPT_Wide[-1] != 0, 'last')])
End.date.NJPT
...
##Find z -- vector containing the normalized phenotypic value 
Kurt.norm.NJPT<-as.vector(Kurtosis.NJPT-mean(Kurtosis.NJPT))/sd(Kurtosis.NJPT)
Skew.norm.NJPT<-as.vector(Skewness.NJPT-mean(Skewness.NJPT))/sd(Skewness.NJPT)
Var.norm.NJPT<-as.vector(Variance.NJPT-mean(Variance.NJPT))/sd(Variance.NJPT)
Start.date.norm.NJPT<-as.vector(Start.date.NJPT-mean(Start.date.NJPT))/sd(Start.date.NJPT)
Duration.norm.NJPT<-as.vector(Duration.NJPT-mean(Duration.NJPT))/sd(Duration.NJPT)

#Trait matrix -- Kurtosis
trait.matrix.raw.k.NJPT<-as.matrix(Kurt.norm.NJPT) 
trait.matrix.k.NJPT<- scale(trait.matrix.raw.k.NJPT)
#Rho  -- Kurtosis
...
#Trait matrix -- Skewness
trait.matrix.raw.s.NJPT<-as.matrix(Skew.norm.NJPT)
trait.matrix.s.NJPT<- scale(trait.matrix.raw.s.NJPT)
#Rho -- Skewness
...
#Trait matrix -- Start date
trait.matrix.raw.st.NJPT<-as.matrix(Start.date.norm.NJPT) 
trait.matrix.st.NJPT<- scale(trait.matrix.raw.st.NJPT)
#Rho -- Start date
rho.matrix.st.NJPT <- t(trait.matrix.st.NJPT) %*% mating.matrix.NJPT %*% trait.matrix.st.NJPT
rho.matrix.st.NJPT

#Trait matrix -- Duration
trait.matrix.raw.d.NJPT<-as.matrix(Duration.norm.NJPT)
trait.matrix.d.NJPT<- scale(trait.matrix.raw.d.NJPT)
#Rho -- Duration
